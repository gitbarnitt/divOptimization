#' Generate observation-level posterior predictive draws (Tier 2)
#'
#' This function extends manual_posterior_predict() to include residual
#' variation and species covariance from the fitted GJAM model. This provides
#' field-realistic detectability estimates that incorporate both inference
#' uncertainty (β) and observation-level variability (Σ).
#'
...
#' @param eps_sigma Diagonal jitter for PSD safety (default: 1e-8)
...
#' @param return_mean Logical; also return mean-only (Xβ) predictions?
#'
#' @return A numeric array [n_draws, nrow(xnew), n_species] of predicted
#'   observations. If return_mean = TRUE, returns list(mean, obs).
#'
#' @details
#' Tier 2 (field-realistic): Adds residual variability using Σ from sigmaSave.
#' This typically reduces detectability vs Tier 1 (mean-only) and provides
#' more conservative sample size recommendations for operational monitoring.
#'
...
manual_posterior_predict_obs <- function(
  ...
  # Step 1: Construct model matrix X
  ...
  # ---------- Determine formula based on training data NLCD variation ----------
  ...
  if (is.numeric(sigmaSave) && is.null(dim(sigmaSave))) {
    ...
    if (length(sigmaSave) == expected_length) {
      ...
      stop(sprintf(
        "sigmaSave is a flat vector of length %d, expected %d (n_iter=%d, S=%d)",
        length(sigmaSave), expected_length, n_iter, S
      ))
    }
  }
